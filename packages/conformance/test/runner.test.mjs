...
describe("conformance runner", () => {
  it("reports the reference adapter client run as a passing partial evaluation", async () => {
    const report = await runConformance(createReferenceAdapter(), {
    ...
    expect(report.scope.evaluatedBehaviorIds).toEqual(
      clientBehaviors.map((behavior) => behavior.id),
    );
  });

  it("requires the full canonical inventory for a conformant verdict", async () => {
    const adapter = createReferenceAdapter();
    adapter.behaviors = Object.fromEntries(
      BEHAVIORS.map((behavior) => [behavior.id, async () => {}]),
    );

    const report = await runConformance(adapter);

    expect(report.scope.complete).toBe(true);
    expect(report.conformant).toBe(true);
  });

  it("never treats an empty or altered selection as a conformance verdict", async () => {
    const adapter = {
      implementation: "empty-selection",
      store: "Google",
      behaviors: {},
    };
    const empty = await runConformance(adapter, { behaviors: [] });

    expect(empty.results).toEqual([]);
    expect(empty.conformant).toBe(false);
    expect(formatReport(empty)).toContain("PARTIAL evaluation");
    expect(formatReport(empty)).toContain("not a conformance verdict");
    await expect(
      runConformance(adapter, { behaviors: [{ id: "not.a.real.behavior" }] }),
    ).rejects.toThrow(/Unknown conformance behavior/);
  });
  ...
  it("fails an adapter that does not implement a MUST behavior", async () => {
    ...
    expect(report.results.every((result) => result.outcome !== "pass")).toBe(
      true,
    );
  });
  ...
  it("propagates a violated assertion as a failure with its reason", async () => {
    const adapter = createReferenceAdapter();
    adapter.behaviors["identifiers.purchase-carries-a-concrete-store"] =
      async () => {
  ...
  it("marks capability-gated behavior not-applicable for a store that cannot support it", async () => {
    ...
    const alreadyOwned = report.results.find(
      (item) =>
        item.id === "purchases.already-owned-surfaces-already-owned-error",
    );
  ...
  it("still requires capability-gated behavior of a store that must support it", async () => {
    const report = await runConformance(
      createReferenceAdapter({ store: "Google" }),
      {
        behaviors: clientBehaviors,
      },
    );
    ...
    const pending = report.results.find(
      (item) =>
        item.id === "purchases.pending-purchase-is-not-delivered-as-purchased",
    );
  ...
  it("does not require an optional capability that the adapter omits", async () => {
    ...
    const adapter = {
      implementation: "amazon-without-webhooks",
      store: "Amazon",
      behaviors: {},
    };
  ...
  it("rejects an adapter missing its identity", async () => {
    await expect(runConformance({ store: "Google" })).rejects.toThrow(
      /implementation is required/,
    );
    await expect(runConformance({ implementation: "x" })).rejects.toThrow(
      /store is required/,
    );
  });
...
describe("conformance report", () => {
  it("renders a human-readable partial verdict naming both versions", async () => {
    const report = await runConformance(createReferenceAdapter(), {
      behaviors: clientBehaviors,
    });
  ...
  it("emits a stable JSON artifact", async () => {
    ...
    expect(parsed.scope.complete).toBe(false);
    expect(parsed.scope.evaluatedBehaviorIds).toEqual(
      clientBehaviors.map((behavior) => behavior.id),
    );
