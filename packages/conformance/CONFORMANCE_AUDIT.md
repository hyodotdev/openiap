...
| `specs/openiap/client/src/schema-contract.test.ts` | Schema contract | GraphQL directive/type-shape assertions (legitimately schema validation) |
...
The canonical spec is the GraphQL schema set in `specs/openiap/client/src/` — 9 files, 3,224 lines:
...
Generation runs through two guarded lanes (`specs/openiap/client/package.json` → `generate`): graphql-codegen for TypeScript, and a custom Parser → IR → language-plugin pipeline for Swift, Kotlin, Dart, GDScript, and C#. Output lands in `specs/openiap/client/src/generated/` and is distributed by `specs/openiap/client/generated-sync-manifest.mjs` to eight targets (Apple `Types.swift`, Google `Types.kt`, and the six framework SDKs' generated type files).
...
| --- | --- | --- |
| `specs/openiap/client` | 20 | Schema + codegen validation |
...
| --- | --- | --- |
| `specs/openiap/client/src/schema-contract.test.ts` | Directive locations, union allowlist, `platform` field removed from `PurchaseAndroid`/`PurchaseIOS` in favor of `store` | Schema validation |
| `specs/openiap/client/src/generated-compatibility.test.ts` | Deprecation tags, doc-comment preservation, blank-line formatting, published-signature stability across generated languages | Code-generation validation |
| `specs/openiap/client/src/generated-sync-verifier.test.mjs`, `generated-sync-manifest.test.mjs` | Generated files match manifest targets | Drift detection |
| `specs/openiap/client/src/schema-linter.test.ts`, `schema-*.test.mjs` | Schema hygiene, deprecation markers | Schema validation |
...
Files: `specs/openiap/client/src/error.graphql`, `packages/apple/Sources/Models/OpenIapError.swift`, `conformance/spec/behaviors/errors.yaml`. Dependencies: none.
...
Files: `specs/openiap/client/src/capability.graphql`, `specs/openiap/client/codegen/`, all 6 language plugins, `generated-sync-manifest.mjs`. Dependencies: none. Note: the existing drift gates (`assert-clean-worktree.mjs`) make this schema change mechanically safe.
...
Files: `packages/kit/convex/purchases/{ios,android,amazon,horizon}.ts`, `shared.ts`, `specs/openiap/client/src/api.graphql` (result union). Dependencies: none.
...
Files: all `specs/openiap/client/src/*.graphql`, `specs/openiap/client/src/schema-linter.test.ts`. Dependencies: none, but P1-1 shares the tooling.
...
**No.** It can honestly say something narrower and still valuable: *OpenIAP implementations are type-conformant and API-surface-complete against a shared GraphQL specification, enforced in CI on every pull request.* That claim is well-supported by `specs/openiap/client/`, the six-language codegen pipeline, `generated-sync-manifest.mjs`, the clean-worktree drift gates, and `audit-non-godot-parity.mjs`.
...
| 6 | Machine-checked store capability matrix | `specs/openiap/client/src/capability-matrix.mjs`, `capability-matrix.test.ts` |
...
`specs/openiap/client/src/capability-matrix.mjs` makes §8's prose machine-checkable: ten
...
| `bun run test` (specs/openiap/client) | **171 passed**, 20 files (+10 capability tests) |
...
| `specs/openiap/client` | **171 passed** |
...
| Suite | Result |
| --- | --- |
| `specs/openiap/client` | **171 passed** (regenerated schema + updated deprecation tests) |
| `packages/conformance` | **20 passed** |
| `packages/kit` | **1187 passed**, 1 skipped |
...
import { capabilityLevel } from '../../../../specs/openiap/client/src/capability-matrix.mjs';
...
Swift behavior ids. `specs/openiap/client` remains the source of truth; the package no longer reads
