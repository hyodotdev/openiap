...
describe('Kotlin platform post-processing', () => {
  it('reproduces the checked-in Google target exactly', () => {
    expect(postProcessKotlinSource(canonical, 'google')).toBe(read(GENERATED_SYNC_MANIFEST.kotlin.targets.google.path));
  });

  it('reproduces the checked-in KMP target exactly', () => {
    expect(postProcessKotlinSource(canonical, 'kmp')).toBe(read(GENERATED_SYNC_MANIFEST.kotlin.targets.kmp.path));
  });
...
`;
    ...
    expect(google).toContain('@file:Suppress("UNCHECKED_CAST")\npackage dev.hyo.openiap');
    ...
    expect(kmp).toContain('@file:Suppress("UNCHECKED_CAST")\n\npackage io.github.hyochan.kmpiap.openiap');
  ...
  it('rejects unknown profiles and multiple package declarations', () => {
    expect(() => postProcessKotlinSource(canonical, 'unknown')).toThrow('Unknown Kotlin platform post-process profile');
    expect(() => postProcessKotlinSource('package one\npackage two\n', 'kmp')).toThrow('multiple package declarations');
  });
...
`;
    ...
    expect(() => postProcessKotlinSource(fixture, 'google')).toThrow('ExampleValue is missing fromJson when(value) parsing');
  });
...
`;
    ...
    expect(() => postProcessKotlinSource(fixture, 'google')).toThrow('ExampleValue.FirstValue raw value "first-value" does not round-trip');
  });
...
`;
    ...
    expect(() => postProcessKotlinSource(fixture, 'google')).toThrow('maps alias "legacy-value" to unknown constant MissingValue');
  });
});
