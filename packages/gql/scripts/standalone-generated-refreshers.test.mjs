...
const packageRootFor = ({ scriptPath }) => scriptPath.slice(0, scriptPath.indexOf('/scripts/'));
...
function createIsolatedCheckout(definition, { withVersions = true } = {}) {
  ...
  const isolatedScript = join(packageRoot, 'scripts', basename(definition.scriptPath));
...
function runRefresher(definition, checkout, { args = [], body = definition.fixture, curlExit = '0' } = {}) {
...
function assertNoRefreshTemps(checkout) {
  ...
  assert.equal(
    parentEntries.some((entry) => entry.includes('.tmp.') || entry.startsWith('.openiap-types-')),
    false,
  );
}
...
test('standalone generated refreshers stay linked to manifest targets', () => {
  for (const definition of refreshers) {
    const source = readFileSync(resolve(repositoryRoot, definition.scriptPath), 'utf8');
  ...
  assert.doesNotMatch(
    readFileSync(resolve(repositoryRoot, 'libraries/godot-iap/scripts/generate-types.sh'), 'utf8'),
...
test('standalone refreshers validate and atomically replace in isolation', () => {
  for (const definition of refreshers) {
    ...
    assert.equal(success.status, 0, `${definition.scriptPath}\n${success.stderr}`);
    ...
    assert.equal(idempotent.status, 0, `${definition.scriptPath}\n${idempotent.stderr}`);
    assert.equal(readFileSync(checkout.isolatedTarget, 'utf8'), expected);
    ...
    assert.equal(readFileSync(checkout.isolatedTarget, 'utf8'), 'preserve-invalid\n');
    assertNoRefreshTemps(checkout);
    ...
    assert.equal(readFileSync(checkout.isolatedTarget, 'utf8'), 'preserve-download\n');
    assertNoRefreshTemps(checkout);
  }
});

test('Node refreshers keep explicit tag overrides independent of metadata', () => {
  for (const definition of refreshers.filter(({ runtime }) => runtime === 'node')) {
    const checkout = createIsolatedCheckout(definition, {
      withVersions: false,
    });
    writeFileSync(checkout.isolatedTarget, 'preserve-me\n', { mode: 0o644 });
