...
const SCHEMA_DIR = 'packages/gql/src';
...
export function schemaFileNamesAtRef(ref, root = repositoryRoot) {
  try {
    const names = parseSchemaFileNames(
      runGit(['show', `${ref}:packages/gql/schema-files.mjs`], root),
    );
    if (names.length > 0) return names;
  } catch {
    // Inventory file absent at the ref — use the current list.
  }
  return SCHEMA_FILE_NAMES;
}

export function readSdlAtRef(ref, root = repositoryRoot) {
  const parts = schemaFileNamesAtRef(ref, root).map((name) => {
    try {
      return runGit(['show', `${ref}:${SCHEMA_DIR}/${name}`], root);
    } catch {
