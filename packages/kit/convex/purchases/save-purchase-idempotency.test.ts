...
describe("savePurchaseInternal — releasing a dead binding", () => {
  async function seedBoundAmazon(db: MemDb, ctx: ReturnType<typeof makeCtx>) {
    const amazon = buildArgs({
      store: "amazon",
      remoteId: "production:viewer:receipt-1",
      requestData: {
        store: "amazon",
        userId: "viewer",
        receiptId: "receipt-1",
    ...
    const [row] = await db.query("purchases").collect();
    await db.patch(row._id, { appUserId: "alice" });
    return amazon;
  }

  // Twenty refunded receipts would otherwise hold the per-user bind cap for
  // good, and a confirming recheck returns before any patch.
  it.each([
    HarmonizedPurchaseState.CANCELED,
    HarmonizedPurchaseState.INAUTHENTIC,
  ])(
    "releases the binding on a terminal Amazon verdict (%s)",
    async (state) => {
      const db = new MemDb();
      db.seedOrg(ORG_ID);
      db.seedProject(PROJECT_ID, ORG_ID);
      const ctx = makeCtx(db);
      const amazon = await seedBoundAmazon(db, ctx);
      await savePurchaseInternal({
        ctx,
        ...amazon,
        persistIfChanged: true,
      ...
      const [row] = await db.query("purchases").collect();
      expect(row.appUserId).toBeUndefined();
    },
  );

  it("releases it again when a later recheck only confirms the verdict", async () => {
    const db = new MemDb();
    db.seedOrg(ORG_ID);
    db.seedProject(PROJECT_ID, ORG_ID);
    const ctx = makeCtx(db);
    ...
    await savePurchaseInternal({ ctx, ...canceled });
    let [row] = await db.query("purchases").collect();
    await db.patch(row._id, { appUserId: "alice" });
    // The verdict is unchanged, so the write is skipped — the release is not.
    await savePurchaseInternal({ ctx, ...canceled, persistIfChanged: true });
    [row] = await db.query("purchases").collect();
    expect(row.appUserId).toBeUndefined();
  });

  it("keeps a Horizon binding, whose row the same customer reuses", async () => {
    const db = new MemDb();
    db.seedOrg(ORG_ID);
    db.seedProject(PROJECT_ID, ORG_ID);
    const ctx = makeCtx(db);
    ...
    [row] = await db.query("purchases").collect();
    expect(row.appUserId).toBe("alice");
  });
});
