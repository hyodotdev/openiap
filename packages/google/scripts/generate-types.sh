...
SOURCE_FILE="${GQL_DIR}/src/generated/Types.kt"
...
echo "📦 Generating Kotlin types in gql package..."
cd "$GQL_DIR"
bun run generate:kotlin

# Check if source file was generated
if [[ ! -f "$SOURCE_FILE" ]]; then
  echo "Error: Types.kt not found at $SOURCE_FILE" >&2
  echo "Generation may have failed" >&2
  exit 1
fi

# Copy to android package
echo "📋 Copying Types.kt to android package..."
mkdir -p "${TARGET_DIR}"
cp "${SOURCE_FILE}" "${TARGET_FILE}"

# Post-process the file (same as original script)
...
echo "✅ Types.kt updated at ${TARGET_FILE}"
