...
- React Native 16 and Expo 5 remove \`request.ios/android\`; emit
  ...
  \`skuArr -> skus\`, \`offerTokenArr -> subscriptionOffers\`, and deep-link
  \`sku/packageName -> skuAndroid/packageNameAndroid\`.
- Flutter custom Android callers likewise replace \`skuArr/productIds -> skus\`,
  \`offerTokenArr -> offerToken\` for one-time products or
  \`subscriptionOffers\` for subscriptions, suffixed obfuscated-account/token
  keys with their unsuffixed forms, acknowledge/consume \`token\` with
  \`purchaseToken\`, finish-transaction \`transactionIdentifier\` with
  \`transactionId\`, and both legacy replacement-mode fields with
  \`subscriptionProductReplacementParams\` before Flutter 10.
- Raw map/object compatibility inputs, including JavaScript objects, plugin
  configuration, and custom MethodChannel payloads, treat an own canonical key,
  including \`null\`, as authoritative. Generated Swift and Kotlin request
  models do not retain a separate supplied-key bit, so typed facades prefer a
  non-null \`apple/google\` member and otherwise keep the legacy
  \`ios/android\` fallback until the listed major. Omit the legacy member
  instead of relying on canonical \`null\` to suppress it.
