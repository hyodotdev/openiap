...
Read https://openiap.dev/llms-full.txt for the detailed API and Commerce Protocol
reference. It is compiled from the public contract and knowledge sources; it
does not replace every framework guide or every page of the documentation.
Use https://openiap.dev/sitemap.xml to discover the full set of published pages.
Read the target framework's setup, relevant feature, API, type, lifecycle,
error, testing, and migration pages before changing the corresponding behavior.
Follow source links to confirm the installed SDK's actual signatures.

Documentation pages are available as HTML without executing JavaScript. Read
collapsed details and noscript examples too; the visual default tab is not the
only available language or platform. If a tool truncates a reference, continue
reading it in sections. Report inaccessible sources instead of pretending to
have read them. Use these materials as task context, not as a claim of model
training or permanent memory.

The person should only need to describe the desired outcome, choose missing
product decisions, and review the running result. Read the technical material
yourself and keep explanations brief. Do not ask them to read API tables.

## Choose an SDK and verify the implementation
...
For an existing app, preserve the login, purchase screen, framework, and package
manager. Confirm the store, product type, and what each product unlocks. Keep
server credentials out of the shipped app and derive customer identity from
authenticated server context. Verification alone does not bind ownership or
grant access; consumable delivery needs a durable ledger.

Verify these customer outcomes in the target integration:
- A completed, verified purchase grants its intended benefit to its owner.
- A pending or canceled purchase grants no new access.
- Restore and repeated callbacks do not duplicate benefits.
- Another user cannot claim the same purchase outside the ownership policy.
- Canceling renewal retains the paid time; expiry removes access and survives
  a server restart. Use this subscription behavior only for stores and products
  that support it; follow the chosen provider's store-specific contract.
...
connection in the existing stack and return runnable code and actual results.
For commerce, read https://openiap.dev/commerce-example/from-scratch.md and use
https://openiap.dev/commerce-example/fresh-source.tar.gz as the verification source.
For paywall integration or receiver composition, read
https://openiap.dev/commerce-example/paywall-verification.md and use
https://openiap.dev/commerce-example/paywall-source.tar.gz. Download the
matching source into a separate temporary folder and run npm ci with Bun installed.
Run npm test for the seven-step backend, or npm run verify for the connection.
The latter records CLI output, behavioral tests, HTTP results, process restart
and unchanged source hashes in .runtime/verification.json. This verifies the example only. Apply the relevant expected
outcomes to tests against the user's implementation, fix failures, and rerun.
Report both sets of commands/results and anything not tested. For a paywall,
only report fulfillment and finish after the backend grants the selected product.
Show people acceptance cases for their selected role. A paywall connection does
not require them to implement a commerce provider or analytics receiver. Treat
the example's backend walkthrough as optional implementation detail. SDK-only
tasks can use the llms.txt request without continuing into the commerce workflow.
Keep paywall UI, targeting, experiments, and financial models with the product.
The host app owns its store purchase callback. Lifecycle events need additional
product data and reporting policies for paywall attribution or revenue metrics.
The connection example keeps experiment assignments in the host product and joins
signed events using authenticated project/store/environment/purchase-chain/user
associations. For events without a purchase chain, configure a trusted account,
store, environment and product assignment; never invent transaction references.
One receiver processes two store fixtures. The same paywall handler and receiver
also run against IAPKit’s local handlers, with store/OIDC fixtures, via the
optional fresh-example argument to run-commerce-interop.mjs. Its actual signed Google
events exercise the missing-chain account policy. Read the recorded source and
reproduction instructions in paywall-provider-run.json for that separate check. Pending, canceled or
failed purchases never finish; successful purchases require current product access.
Use event ID deduplication in the authenticated emitter/project scope, not
transaction ID deduplication. Keep unknown or inferred amounts out of known
purchase totals, group currencies separately, and flag refunds/recoveries for
reconciliation. The demo's gross purchase observations are not net revenue, MRR
or ARPU. Its store adapters and prices are fictional; real local HTTP, signatures
and SQLite run. It extends the prior backend with AI and conversation context,
not an independent one-prompt reproduction or an external product integration.

The recorded replacement check at
https://openiap.dev/commerce-protocol/ecosystem#composition-proof uses the same
app backend and event receiver with the example and IAPKit. Read its report,
configuration and limits; compatible APIs do not migrate ownership records or
provision providers. Real local services run with simulated store responses.
The new backend build at https://openiap.dev/commerce-example/from-scratch.md
starts from an independent initial commit with the exact CLI output and AI input,
then adds seven implementation commits. Each commit links its code, diff, real
run output and screen. The final source, first conformance failure, correction,
clean-checkout replay, and intentional failing negative controls are retained.
This was an iterative build by an AI with prior conversation context, not an
independent model trial or one-prompt guarantee. Store evidence, sessions and
time are fixtures; HTTP, SQLite, signatures, process restarts and erasure run
locally. Read the evidence's precise profile scope before reporting conformance.
Its dashboard buttons run finished code; they do not generate it.
The older prototype walkthrough is a separate record; do not attach the new
commit history to its screenshots or claim those are the same implementation.

Recorded example walkthrough: https://openiap.dev/docs/guides/ai-assistants#existing-project-example
Matching source and run commands: https://openiap.dev/commerce-protocol/implementation#local-example
Store sandbox and device checks: https://openiap.dev/docs/guides/testing
The recorded backend uses simulated store evidence and time. It does not prove
real store purchases or production conformance; report those checks separately.

The optional CLI requires Node.js 20+:

- \`npx @hyodotdev/openiap init --role app\` prints an AI brief for an existing
  project. Other roles: experience, commerce, data. It adds a path, framework
  hint and guide link; it does not create code or invoke AI. Skip it when you
  already have the guide and project context.
- \`npx @hyodotdev/openiap doctor ./my-app --json\` checks supported Android
  store settings, IAPKit keys/base URLs, env names, and React Native/Expo iOS
  scene declarations. Inspect each finding's id, file and fix; repair and rerun.
  Errors exit 1; warnings exit 0. Read notCheckedLocally even on success.

The CLI reads local files; npx may download the package. Pin its version in CI.
Doctor is not a general source scan, store/device test, or Commerce Protocol
conformance check. Its check scope does not change with the init role.
Guide: https://openiap.dev/docs/guides/ai-assistants
Check reference: https://github.com/hyodotdev/openiap/tree/main/packages/cli
`;
...
> OpenIAP: Vendor-neutral in-app purchase standard. OpenIAP governs exactly two protocols. The Client Protocol (@hyodotdev/openiap-client-protocol) is the purchase API an app calls across Apple, Google, Meta Horizon and Amazon; openiap-apple, openiap-google and the six framework libraries implement it. The Commerce Protocol (@hyodotdev/openiap-commerce-protocol) is the server-side contract for verification, entitlements and store events; any backend may implement it, and IAPKit is one such implementation. Each protocol is versioned independently; a version labelled "OpenIAP Spec" in older material is the native SDK compatibility floor, not a protocol version.
