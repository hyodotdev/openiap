...
function checkFrameworkDependencyHygiene() {
  ...
  const horizonPlatformVersions = uniqueMatches(
    googleBuildGradle,
    /val\s+horizonPlatformVersion\s*=\s*"([^"]+)"/g,
  );
  ...
  if (horizonPlatformVersions.length !== 1) {
    fail(
      `packages/google must use one Horizon Platform SDK version, found: ${horizonPlatformVersions.join(', ') || '(none)'}`,
    );
  }
  ...
  const mauiAmazonAppstoreSdkVersion = mauiProps.match(/<MauiAmazonAppstoreSdkVersion>([^<]+)<\/MauiAmazonAppstoreSdkVersion>/)?.[1];
  const mauiHorizonPlatformSdkVersion = mauiProps.match(/<MauiHorizonPlatformSdkVersion>([^<]+)<\/MauiHorizonPlatformSdkVersion>/)?.[1];
  const mauiHorizonBillingCompatibilityVersion = mauiProps.match(/<MauiHorizonBillingCompatibilityVersion>([^<]+)<\/MauiHorizonBillingCompatibilityVersion>/)?.[1];
  ...
  if (!mauiHorizonPlatformSdkVersion) {
    fail('MAUI Directory.Build.props must define MauiHorizonPlatformSdkVersion');
  }
  if (!mauiHorizonBillingCompatibilityVersion) {
    fail('MAUI Directory.Build.props must define MauiHorizonBillingCompatibilityVersion');
  }
  if (!mauiGsonVersion) {
  ...
  if (amazonAppstoreSdkVersions.length === 1 && mauiAmazonAppstoreSdkVersion !== amazonAppstoreSdkVersions[0]) {
    fail(
      `MAUI Amazon Appstore SDK version ${mauiAmazonAppstoreSdkVersion} must match packages/google ${amazonAppstoreSdkVersions[0]}`,
    );
  }
  if (horizonPlatformVersions.length === 1 && mauiHorizonPlatformSdkVersion !== horizonPlatformVersions[0]) {
    fail(
      `MAUI Horizon Platform SDK version ${mauiHorizonPlatformSdkVersion} must match packages/google ${horizonPlatformVersions[0]}`,
    );
  }
  if (horizonBillingVersions.length === 1 && mauiHorizonBillingCompatibilityVersion !== horizonBillingVersions[0]) {
    fail(
  ...
  expectIncludes('libraries/maui-iap/src/OpenIap.Maui.Bindings.Android/OpenIap.Maui.Bindings.Android.csproj', [
    ...
    'Version="$(MauiHorizonPlatformSdkVersion)"',
