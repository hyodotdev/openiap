...
function assertNoUnmanagedFundingLinks(readme, funding) {
  ...
  const end = readme.indexOf(sponsorBlockEnd, start) + sponsorBlockEnd.length;
...
export function discoverReadmes(
  ...
  if (staged) {
    return execFileSync(
      "git",
      ["ls-files", "--cached", "--", "README.md", "packages", "libraries"],
      ...
      .filter(
        (relative) =>
          relative === "README.md" ||
          /^(?:packages|libraries)\/[^/]+\/README\.md$/u.test(relative),
      )
      .sort();
  }
  ...
  for (const owner of ["packages", "libraries"]) {
...
export function synchronizeSponsorFiles(
  ...
  for (const relative of discoverReadmes(root, { staged })) {
    ...
    try {
      expected = synchronizeReadme(actual, sponsorBlock);
