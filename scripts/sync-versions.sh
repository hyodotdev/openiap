...
PY
    ...
    python3 - "$maui_props" "$play_billing_version" "$amazon_appstore_sdk_version" "$horizon_billing_compatibility_version" "$horizon_platform_kotlin_version" "$gson_version" <<'PY'
import re
import sys

path, play_billing, amazon_appstore_sdk, horizon_billing_compatibility, horizon_platform_kotlin, gson = sys.argv[1:]
