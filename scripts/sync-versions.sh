...
sync_maui_android_versions() {
    ...
    local horizon_platform_sdk_version
...
PY
    ...
    horizon_platform_sdk_version=$(python3 - "$google_build" <<'PY'
import re
import sys

text = open(sys.argv[1], encoding="utf-8").read()
match = re.search(r'val\s+horizonPlatformVersion\s*=\s*"([^"]+)"', text)
if not match:
    raise SystemExit("missing horizonPlatformVersion")
print(match.group(1))
PY
)
...
PY
    ...
    python3 - "$maui_props" "$play_billing_version" "$amazon_appstore_sdk_version" "$horizon_platform_sdk_version" "$horizon_billing_compatibility_version" "$gson_version" <<'PY'
import re
import sys

path, play_billing, amazon_appstore_sdk, horizon_platform_sdk, horizon_billing_compatibility, gson = sys.argv[1:]
...
content = f"""<Project>
  ...
  <PropertyGroup>
    ...
    <MauiHorizonPlatformSdkVersion>{horizon_platform_sdk}</MauiHorizonPlatformSdkVersion>
