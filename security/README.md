...
| Check                                        | Proves                                                                                                                                                                                                                                                                        |
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bun run audit:horizon-app-id`               | every example _declares_ the meta-data under `<application>`, with a literal id or a Gradle placeholder. For Expo it reads the options object the plugin entry names, accepting only plain object literals and a literal id — a spread, ternary, call or computed key is reported rather than resolved, because resolving one means evaluating the config. For the same reason it cannot prove that entry is still in the exported `plugins` array |
