...
const changedInputs = (...args) =>
  gitLines(...args, "--", ...GQL_GENERATION_INPUT_PATHS).filter(
    isGqlGenerationInputPath,
  );
...
const drift = [
  ...changedInputs("diff", "--name-only"),
  ...changedInputs("ls-files", "--others", "--exclude-standard"),
]
...
if (process.argv[2] === "has-staged-inputs") {
  ...
  throw new Error(
    `Unknown command "${process.argv[2] ?? ""}". Expected has-staged-inputs or assert-staged-clean.`,
  );
}
