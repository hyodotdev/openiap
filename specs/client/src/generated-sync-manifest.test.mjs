...
describe("generated sync manifest", () => {
  ...
  it("owns the public client-spec export paths", () => {
    ...
    expect(packageJson.scripts.prepack).toBe("bun run build:package");
    expect(packageJson.files).toContain("dist/");

    for (const definition of definitions) {
      const exportTarget = packageJson.exports[definition.exportKey];
      if (typeof exportTarget === "string") {
        expect(exportTarget, definition.exportKey).toBe(
          `./${gqlPackageRelativePath(definition.source)}`,
        );
        continue;
      }
      // TypeScript entries ship compiled declarations so consumer compiler
      // flags never re-check the authored sources; workspace consumers keep
      // resolving the sources through the openiap-source condition.
      const published = gqlPublishedArtifactPaths(definition.source);
      expect(exportTarget, definition.exportKey).toEqual({
        "openiap-source": `./${gqlPackageRelativePath(definition.source)}`,
        types: published.types,
        import: published.runtime,
        default: published.runtime,
      });
    }
    expect(packageJson.types).toBe("dist/generated/types.d.ts");
    expect(packageJson.typesVersions).toEqual({
      "*": { "kit-api": ["./dist/kit-api.d.ts"] },
    });
  });
